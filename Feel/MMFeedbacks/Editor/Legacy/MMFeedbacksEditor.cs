...
{
	/// <summary>
	/// 一个自定义编辑器，显示可折叠的MMFeedbacks列表，一个用于添加更多信息的下拉列表，以及用于在运行时测试反馈的测试按钮
	...
	public class MMFeedbacksEditor : Editor
	{
		...
		/// <summary>
		/// 绘制检查器，包括帮助框、初始模式选择、反馈列表、反馈选择和测试按钮
		...
		public override void OnInspectorGUI()
		{
			...
			if (!MMFeedbacks.GlobalMMFeedbacksActive)
			{
				...
				EditorGUILayout.HelpBox("包括此反馈在内的所有MMFeedbacks当前都已禁用。这是通过脚本完成的，通过更改MMFeedbacks的值。GlobalMMFeedbackActive布尔值。目前，该值已设置为false。将其设置为true将允许MMFeedbacks再次播放。", MessageType.Warning);
				EditorGUILayout.Space();
				GUI.color = baseColor;
			}
			...
			{
                //The MMFeedbacks component is getting replaced by the new and improved MMF_Player, which will improve performance, let you keep runtime changes, and much more! The MMF_Player works just like MMFeedbacks.Consider using it instead, and don't hesitate to check the documentation to learn all about it.

                EditorGUILayout.HelpBox("MMFeedbacks组件正在被新的改进的MMF_Player所取代，这将提高性能，让您保持运行时更改，等等！MMF_ Player的工作原理与MMF反馈一样。考虑改用它，并且毫不犹豫地查看文档以了解它的全部内容。", MessageType.Warning);    
			}
