...
{
	/// <summary>
	/// 此反馈允许您在反馈位置（加上可选偏移量）实例化在其检查器中指定的对象。您还可以选择（并自动）在初始化时创建对象池，以节省性能。在这种情况下，您需要指定池大小（通常是您计划在每个给定时间在场景中拥有的这些实例化对象的最大数量）。
	...
	/// </summary>
	[AddComponentMenu("")]
	[FeedbackHelp("此反馈允许您在反馈位置（加上可选偏移量）实例化在其检查器中指定的对象。您还可以选择（并自动）在初始化时创建对象池，以节省性能。在这种情况下，您需要指定池大小（通常是您计划在每个给定时间在场景中拥有的这些实例化对象的最大数量）。")]
	[FeedbackPath("GameObject（游戏对象）/Instantiate Object（实例化对象）")]
