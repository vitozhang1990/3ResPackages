...
{
    /// <summary>
    /// 此反馈允许您在初始化、播放、停止或重置时将目标游戏对象的状态从活动更改为非活动（或相反）。对于其中的每一个，您可以指定是要强制状态（活动或非活动），还是切换状态（活动变为非活动，非活动变为活动）。
    ...
    /// </summary>
    [AddComponentMenu("")]
	[FeedbackHelp("此反馈允许您在初始化、播放、停止或重置时将目标游戏对象的状态从活动更改为非活动（或相反）。对于其中的每一个，您可以指定是要强制状态（活动或非活动），还是切换状态（活动变为非活动，非活动变为活动）。")]
	[FeedbackPath("GameObject（游戏对象）/Set Active")]
