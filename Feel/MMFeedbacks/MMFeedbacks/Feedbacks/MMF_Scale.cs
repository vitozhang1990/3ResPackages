...
{
    /// <summary>
    /// 此反馈将在指定的持续时间内（以秒为单位）在3条指定的动画曲线上设置目标比例的动画。可以应用乘法器，该乘法器将乘以每个动画曲线值。
	...
	[FeedbackHelp("此反馈将在指定的持续时间内（以秒为单位）在3条指定的动画曲线上设置目标比例的动画。可以应用乘法器，该乘法器将乘以每个动画曲线值。")]
	public class MMF_Scale : MMF_Feedback
	{
		...
		[Tooltip("反馈运行的模式" +
				 "Absolute : 沿着曲线" +
				 "Additive : 添加到目标的当前比例" +
				 "ToDestination : 将比例设置为目标，无论当前比例是什么")]
		...
		[Tooltip("如果为真，则仅使用AnimateX曲线，并将其应用于所有轴")] 
