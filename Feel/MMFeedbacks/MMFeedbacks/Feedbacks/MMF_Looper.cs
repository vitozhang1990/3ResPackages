...
{
    /// <summary>
    /// 此反馈将MMFeedbacks序列的当前“头”移回列表中上面的另一个反馈。
    /// 头部的反馈取决于您的设置：您可以决定在最后一次暂停时循环，或在列表中的最后一次循环开始反馈时循环（或两者都循环）。
    /// 此外，您可以决定让它循环多次，并在遇到时引起暂停。
    ...
    /// </summary>
    [AddComponentMenu("")]
	[FeedbackHelp("此反馈将MMFeedbacks序列的当前“头”移回列表中上面的另一个反馈。" +
        "头部的反馈取决于您的设置：您可以决定在最后一次暂停时循环，或在列表中的最后一次循环开始反馈时循环（或两者都循环）。" +
        "此外，您可以决定让它循环多次，并在遇到时引起暂停。")]
	[FeedbackPath("Loop/Looper（循环）")]
