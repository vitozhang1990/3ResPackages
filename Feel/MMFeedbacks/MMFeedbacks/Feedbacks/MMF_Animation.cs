...
{
	/// <summary>
	/// 用于在相关animation上触发动画（bool、int、float或trigger）的反馈，具有或不具有随机性
	/// A feedback used to trigger an animation (bool, int, float or trigger) on the associated animator, with or without randomness
	/// </summary>
	[AddComponentMenu("")]
	[FeedbackHelp("用于在相关animation上触发动画（bool、int、float或trigger）的反馈，具有或不具有随机性")]
	[FeedbackPath("GameObject（游戏对象）/Animation（动画）")]
