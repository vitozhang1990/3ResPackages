...
{
	/// <summary>
	/// 此反馈将允许您更改相机的优先级。这需要一点设置：为不同的摄像机添加一个MMCinemachinePriorityListener，并在其上添加唯一的频道值。
	...
	#endif
	[FeedbackHelp("此反馈将允许您更改相机的优先级。这需要一点设置：为不同的摄像机添加一个MMCinemachinePriorityListener，并在其上添加唯一的频道值。" +
		"或者，您可以在Cinemachine Brain上添加MMCinemachinePriorityBrainListener，以处理不同的转换类型和持续时间。" +
		"然后你所要做的就是在你的反馈中选择一个频道和一个新的优先级，并播放它。神奇的转变！")]
