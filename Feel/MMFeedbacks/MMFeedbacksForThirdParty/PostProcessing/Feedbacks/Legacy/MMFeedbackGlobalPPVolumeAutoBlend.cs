...
{
	/// <summary>
	/// 此反馈将使您可以试用全局后处理卷组件。GPPVAB组件放置在后处理卷上，可以根据需要控制和混合其重量。
	...
	/// </summary>
	[AddComponentMenu("")]
	[FeedbackHelp("此反馈将使您可以试用全局后处理卷组件。GPPVAB组件放置在后处理卷上，可以根据需要控制和混合其重量。")]
	[FeedbackPath("PostProcess（后处理）/Global PP Volume Auto Blend（试用全局后处理卷组件）")]
