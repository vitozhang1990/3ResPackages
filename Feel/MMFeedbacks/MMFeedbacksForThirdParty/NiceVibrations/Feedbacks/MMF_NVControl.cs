...
{
	/// <summary>
	/// 添加此反馈以在全局级别与触觉交互，停止它们，启用或禁用它们，调整它们的全局级别或初始化/释放触觉引擎
	...
	[AddComponentMenu("")]
	#if MOREMOUNTAINS_NICEVIBRATIONS_INSTALLED
	[FeedbackPath("Haptics（触觉反馈）/Haptic Control（全局触觉控制器）")]
	#endif
	[FeedbackHelp("添加此反馈以在全局级别与触觉交互，停止它们，启用或禁用它们，调整它们的全局级别或初始化/释放触觉引擎")]
