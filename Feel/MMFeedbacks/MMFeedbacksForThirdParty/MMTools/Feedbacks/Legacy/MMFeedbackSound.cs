...
{
	/// <summary>
	/// 通过此反馈，您可以通过事件（您需要场景中的某些内容来捕捉MMSfxEvent，例如MMSoundManager）或缓存（音频源在初始化时创建，然后准备播放）或按需（在播放时实例化）播放指定的音频剪辑。对于所有这些方法，您可以在最小/最大边界（如果不需要随机性，只需在两个字段中设置相同的值）、随机音高和可选的音频混音器组之间定义随机音量。
	/// </summary>
	[ExecuteAlways]
	[AddComponentMenu("")]
	[FeedbackPath("Audio（音效）/Sound（事件播放音频剪辑）")]
	[FeedbackHelp("通过此反馈，您可以通过事件（您需要场景中的某些内容来捕捉MMSfxEvent，例如MMSoundManager）或缓存（音频源在初始化时创建，然后准备播放）或按需（在播放时实例化）播放指定的音频剪辑。对于所有这些方法，您可以在最小/最大边界（如果不需要随机性，只需在两个字段中设置相同的值）、随机音高和可选的音频混音器组之间定义随机音量。")]
