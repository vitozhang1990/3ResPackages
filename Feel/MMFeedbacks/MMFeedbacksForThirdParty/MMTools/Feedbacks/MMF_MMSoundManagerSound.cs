...
{
	/// <summary>
	/// 此反馈将允许您通过MMSoundManager播放声音。你需要在你的场景中有一个游戏对象，上面有一个MMSoundManager对象，这样才能工作。
	...
	[ExecuteAlways]
	[AddComponentMenu("")]
	[FeedbackPath("Audio（音效）/MMSoundManager Sound（播放声音）")]
	[FeedbackHelp("此反馈将允许您通过MMSoundManager播放声音。你需要在你的场景中有一个游戏对象，上面有一个MMSoundManager对象，这样才能工作。")]
