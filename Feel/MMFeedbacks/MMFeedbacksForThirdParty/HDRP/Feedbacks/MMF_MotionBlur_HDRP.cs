...
{
	/// <summary>
	/// 此反馈允许您随时间控制HDRP运动模糊强度。它要求场景中有一个体积为MotionBlur活动的对象和一个MMMotionBlurShaker_HDRP组件。
	...
	[AddComponentMenu("")]
#if MM_HDRP
	[FeedbackPath("PostProcess（后处理）/Motion Blur HDRP（HDRP运动模糊强度）")]
#endif
	[FeedbackHelp("此反馈允许您随时间控制HDRP运动模糊强度。它要求场景中有一个体积为MotionBlur活动的对象和一个MMMotionBlurShaker_HDRP组件。")]
